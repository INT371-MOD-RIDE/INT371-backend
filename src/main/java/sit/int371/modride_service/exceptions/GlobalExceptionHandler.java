...
import sit.int371.modride_service.exceptions.ApiError;
...
public class GlobalExceptionHandler extends BaseController {
    ...
    public ResponseEntity<APIResponseBean> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
            HttpServletResponse response) {
        APIResponseBean res = new APIResponseBean();
        String errorMessage = ex.getBindingResult().getAllErrors().get(0).getDefaultMessage();
        // response.setStatus(UnprocessableContentStatus);
        res.setResponse_code(UnprocessableContentStatus);
        res.setResponse_desc(errorMessage);
        return ResponseEntity.status(UnprocessableContentStatus).body(res);
    }
    ...
    public ResponseEntity<Object> handleConstraintViolation(ConstraintViolationException ex, WebRequest request) {
        ...
        // ApiError apiError500 = new
        // ApiError(HttpStatus.INTERNAL_SERVER_ERROR,ex.getLocalizedMessage(),errors);
        // return new ResponseEntity<Object>(apiError500, new HttpHeaders(),
        // apiError500.getStatus());
    }
