...
import sit.int371.modride_service.dtos.EditEventDTO;
import sit.int371.modride_service.dtos.EventDTO;
import sit.int371.modride_service.dtos.NewEventDTO;
import sit.int371.modride_service.entities.*;
import sit.int371.modride_service.repositories.*;
import sit.int371.modride_service.services.DBFileService;
import sit.int371.modride_service.services.EmailService;
import sit.int371.modride_service.services.EventService;
...
public class EventController {
    ...
    @Autowired
    EventCateOwnRepository eventCategoryOwnerRepository;


    public EventController(EventRepository repository, OldUserRepository OldUserRepository, EventCateOwnRepository eventCategoryOwnerRepository) {
            ...
            if (tokenDecoded.getAlgorithm().contains("RS256")) {
                ...
                if (tokenDecoded.getClaims().get("roles") == null) {
                    ...
                    if (msRole.contains(admin) || msRole.contains(student)) {
                        ...
                        User user = OldUserRepository.findByEmail(msEmail.replaceAll("^\"|\"$", ""));
                        if (user == null){
...
//            token from oasip system
                ...
                User user = OldUserRepository.findByEmail(lecEmail);
//        ขั้นตอนนี้จะ
...
//lecturer-role
                        ...
                        System.out.println("ms [lecturer] role : " + msRole);
                        User lecUser = OldUserRepository.findByEmail(msEmail.replaceAll("^\"|\"$", ""));
...
//        สำหรับกรองข้อมูลของ lecturer
                String lecEmail = SecurityContextHolder.getContext().getAuthentication().getName();
                User lecUser = OldUserRepository.findByEmail(lecEmail);
    ...
    public List<EventDTO> getByEventCategory(@PathVariable Integer eventCategoryId) {
        ...
        String email = SecurityContextHolder.getContext().getAuthentication().getName();
        User user = OldUserRepository.findByEmail(email);
...
//token from oasip
                ...
                //get role from token
                String role = SecurityContextHolder.getContext().getAuthentication().getAuthorities().toString();
                User userEmail = OldUserRepository.findByEmail(email);
